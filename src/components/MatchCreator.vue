<template>
  <div>
    <div class="text-center py-3 bg-neutral-800">
      <h1 class="uppercase font-medium text-xl text-neutral-200">
        New Skinsmatch
      </h1>
    </div>
    <div class="bg-neutral-600 px-2 py-2">
      <div class="grid grid-cols-6 gap-3">
        <div class="col-span-2">
          <label for="holes" class="block text-sm font-medium uppercase"
          >Holes</label
          >
          <input
              autocomplete="off"
        ...
        <div class="col-span-2">
          <label for="payout" class="block text-sm font-medium uppercase"
          >$ per Skin</label
          >
          <div class="mt-1 flex">
            <span class="inline-flex items-center px-3 bg-neutral-800 text-sm">
              $
            </span>
            <input
                autocomplete="off"
        ...
        <div class="col-span-2">
          <label for="starthole" class="block text-sm font-medium uppercase"
          >Start hole</label
          >
          <div class="mt-1 flex">
            <input
      ...
      <div v-for="player in players" :key="player.id">
        <div class="flex">
          <span class="flex-1 block w-full rounded-none text-sm uppercase">
            {{ player.name }}
          </span>
          <span
      ...
      <form class="flex mt-1" v-on:submit.prevent="addPlayer">
        ...
        <span
            @click="addPlayer"
            class="inline-flex items-center px-3 bg-neutral-800 hover:bg-neutral-700 focus:ring-2 focus:ring-offset-2 focus:bg-neutral-600"
        >
          +
        </span>
      </form>
      ...
      <div class="mt-3">
        <div class="relative flex items-start">
          ...
          <div class="ml-3 text-sm">
            <label for="comments" class="text-sm font-medium uppercase"
            >Randomize first tee order</label
            >
          </div>
        </div>
      </div>
      ...
      <div class="mt-4" v-if="this.players.length > 0">
        ...
        >buy in: ${{ sumPerPlayer }} per player / ${{ sumPerPlayerPerHole }} per player per hole/extra skin </span
        >
      </div>
...
export default {
  ...
  computed: {
    sumPerPlayer() {
      if (this.players.length > 0) {
        return (
            Math.round((100 * this.payout * this.holes) / this.players.length) /
            100
        );
      }
      return "-";
    ...
    sumPerPlayerPerHole() {
      if (this.players.length > 0) {
        return (
            Math.round((100 * this.payout) / this.players.length) /
            100
        );
      }
      return "-";
  ...
  methods: {
    ...
    nameInUse() {
      for (let i = 0; i < this.players.length; i++) {
        if (
            this.players[i].name.toLowerCase() ===
            this.newPlayerName.toLowerCase()
        ) {
          return true;
        }
      }
    ...
    startGame() {
      ...
      if (
          parseInt(this.starthole) <= 0 ||
          parseInt(this.starthole) > parseInt(this.holes)
      ) {
